...
class upload {
     ...
     * @access private
     */
	 var $moi_video_to_flv;

    /**
     * Indica si el archivo subido debe ser convertido a flv usando la librería mencoder
     *
    ...
    function process($server_path = null) {
        ...
        if ($this->processed) {
            ...
            if ($image_manipulation) {
				...
				//$this->error = "copy(".$this->file_src_pathname.", ".$this->file_dst_pathname.")";
                if($this->moi_video_to_flv){
					$file_dest = explode('.',$this->file_dst_pathname);
					$file_dest[1] = strtolower($file_dest[1]);
					switch($file_dest[1]){
						case '3gp':
							$cmd = "mencoder ".$this->file_src_pathname." -ofps	15 -of lavf -oac mp3lame -lameopts abr:br=32 -srate 22050 -ovc lavc -lavcopts vcodec=flv:vbitrate=150 -vf scale=320:-3 -o ".$file_dest[0].".flv";
						break;
						
						default:
							$cmd = "mencoder ".$this->file_src_pathname." -ofps 25 -of lavf -oac mp3lame -lameopts abr:br=56 -srate 22050 -ovc lavc -lavcopts vcodec=flv:vbitrate=400:trell:v4mv:cbp:last_pred=3 -vf harddup,scale=480:271 -o ".$file_dest[0].".flv";
						break;
					}
